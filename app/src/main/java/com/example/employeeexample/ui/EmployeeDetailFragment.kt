...
class EmployeeDetailFragment : Fragment() {
    ...
    private fun saveEmployee(){
        ...
        if(name.isBlank()){
            requireActivity().showToast(getString(R.string.employee_validate_error_message, "name"))
            return
        }
        if(responsibilities.isBlank()){
            requireActivity().showToast(getString(R.string.employee_validate_error_message, "responsibilities"))
            return
        }
        if(experience.isBlank()){
            requireActivity().showToast(getString(R.string.employee_validate_error_message, "experience"))
            return
        }
        if(education.isBlank()){
            requireActivity().showToast(getString(R.string.employee_validate_error_message, "education"))
            return
        }
        val phone: Long
        try {
            ...
            requireActivity().showToast(getString(R.string.employee_validate_error_message, "valid phone"))
            return
        }

        if(email.isBlank()){
            requireActivity().showToast(getString(R.string.employee_validate_error_message, "email"))
            return
        }
        //simple email validation
        if(email.indexOf("@") < 0){
            requireActivity().showToast(getString(R.string.employee_validate_error_message, "valid email"))
            return
        }
        if(address.isBlank()){
            requireActivity().showToast(getString(R.string.employee_validate_error_message, "address"))
            return
        }
